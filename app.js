...
let login = require('./data/login.json');
let info = require('./data/info.json');
let code = require('./data/code.json');
let all = require('./data/all.json');
let enabled = require('./data/enabled.json');
let dictDetail = require('./data/dictDetail.json');
let dep = require('./data/dep/dept.json');
let IndexTime = require('./data/bigData/IndexTime.json');
let TopRoad = require('./data/bigData/TopRoad.json');
let TopTime = require('./data/bigData/TopTime.json');
...
router.get('/api/menus/build/all', async ctx => {
    ctx.body =all
});
router.get('/api/system/subsystem/enabled', async ctx => {
    ctx.body =enabled
});
// 字典
router.get('/api/dictDetail', async ctx => {
    let {dictName, page,size} = ctx.request.query;
    ctx.body =dictDetail
});
router.get('/api/dept', async ctx => {
    let {sort} = ctx.request.query;
    ctx.body =dep
});
// 大数据
router.get('/webapi/Alarm/TopRoad', async ctx => {
    let {day,month,year} = ctx.request.query;
    ctx.body =TopRoad
});
router.get('/webapi/Alarm/IndexTime', async ctx => {
    let {day,month,year,roadname} = ctx.request.query;
    ctx.body =IndexTime
});
router.get('/webapi/Alarm/TopTime', async ctx => {
    let {month,year} = ctx.request.query;
    ctx.body =TopTime
});
